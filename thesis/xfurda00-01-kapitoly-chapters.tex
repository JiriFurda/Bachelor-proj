...
Tato kapitola v~úvodu definuje pojem webový portál a popisuje několik existujících portálu. Dále přibližuje základní koncepci vývoje webu a praktiky, které jsou při tomto vývoji často využívány. Následně jsou v~této kapitole popsány jazyky a knihovny, využívané při implementaci této bakalářské práce. V samém závěru kapitoly je přiblížena koncepce podobnostního hledání.

\section{Webové portály}
Webový portál je druh webové stránky, která shromažďuje informace z~vícero různých zdrojů a ty nejrelevantnější prezentuje uživatelům shromážděné na jednom místě~\cite{bib:portal-liferay}.
...
Portály lze klasifikovat na tři základní typy - horziontalní, vertikální a podnikové.
Horizontální portály (někdy též označovány jako mega portály) poskytují uživatelům přístup k celé řadě stránek s odlišným zaměřením. Velmi často je uživateli umožněno obsah takového portálu přizpůsobit jeho vlastním potřebám. Příkladem tohoto typu portálu může být například Čechy stále oblíbený portál Seznam\footnote{Seznam: \url{https://www.seznam.cz/}}.   
Vertikální portály se zaměřují na užší spektrum uživatelů, které sdílí společný zájem Obsah takových portálů je zaměřen právě na daný zájem. 
Podnikové portály poskytují obsah sdílený v rámci konkrétní společnosti, přístup k těmto informacím je z pravidla umožněn jen členům společnosti~\cite{bib:portal-use}.
...
Obsah dotazníku vycházel z UEQ\footnote{UEQ: \url{https://www.ueq-online.org/}} (\emph{User Experience Questionnaire}) a respondentům byl prezentován pomocí nástroje Google Formuláře\footnote{Google Formuláře: \url{https://www.google.com/intl/cs_CZ/forms/about/}}.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{obrazky-figures/ueq.pdf}
	\caption{Graf prezentující výsledky dotazníků uživatelského prožitku}
    \label{img:ueq}
\end{figure}

Z grafu~\ref{img:ueq} lze usoudit, že většina aspektů portálu je vnímána pozitivně. Za povšimnutí stojí především výborný výsledek v oblasti přehlednosti, což považuji za velký úspěch. Na druhou stranu dotazník odhalil průměrné hodnoty v oblasti stimulace a poukázal především na slabou stránku portálu a to originalitu.

V souhrnu lze tedy portál označit jako pragmaticky kvalitní, s podprůměrnou hedonickou kvalitou.
...
Pro získání analýzy chování uživatelů byl využit nástroj Smartlook\footnote{Smatlook: \url{https://www.smartlook.com/cs/}}. K analýze bylo díky tomu dostupných hned několik typů tepelných map sledujících buď klikání uživatele, pohyb kurzoru myši nebo rolování po stránce. Navíc bylo možné sledovat nahrávky chování uživatelů a přímo tak pozorovat jejich interakci s portálem.

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{obrazky-figures/heatmap.png}
	\caption{Tepelná mapa klikání uživatelů na hlavní stránce portálu.}
	\label{img:heatmap}
\end{figure}

Výsledky tepelné mapy z obrázku~\ref{img:heatmap}, založené na statistice klikání uživatelů na hlavní stránce, jsou ovlivněny zaznamenáním interakce nejen s vyobrazenou stránkou, ale i částečně i interakcí s modálním oknem, překrývajícím tento obsah. Z toho důvodu se v tepelné mapě vyskytují i zdánlivě nelogické oblasti zájmu uživatelů.

Z mapy je patrné, že fasetová navigace je mezi uživateli velmi oblíbená. Uživatelé dále jeví zájem především o první zobrazený výsledek vyhledávání, část z nich si ale podobněji prohlédne i výsledek umístěný na druhém místě.
