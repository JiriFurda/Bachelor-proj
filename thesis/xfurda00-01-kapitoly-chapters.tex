...
Dále by bylo vhodné vyvolat v uživatelích větší zájem o vyhledávací pole. Toho se lze pokusit docílit například nabízením náhodně vybraných frází, které lze hledat. Tyto fráze by sloužily pouze jako zástupný text (atribut \texttt{placeholder}) a po kliknutí do pole by se skryl. 

Zapracovat by se dalo i na zvýšení viditelnosti níže umístěných výsledků vyhledávání. Pomoci by mohlo kompaktnější prezentování jednotlivých výsledků, alespoň co se týče výšky těchto prvků. Za pokus by stálo prezentovat na jedné stránce více výsledků. Dle průzkumu by tato úprava mohla vést ke značnému zvýšení viditelnosti výsledků, pro jejichž zobrazení je nutné stránku rolovat~\cite{bib:scroll}.
